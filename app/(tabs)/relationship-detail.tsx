            ...
            {isListMode ? (
              <View style={styles.compactProfileHeader}>
                <View style={styles.compactAvatarContainer}>
                  {profile.imageUri ? (
                    <Image source={{ uri: profile.imageUri }} style={styles.avatar} contentFit="cover" />
                  ) : (
                    <View style={styles.avatarPlaceholder}>
                      <MaterialIcons name="person" size={18 * fontScale} color={colors.primaryLight} />
                    </View>
                  )}
                </View>
                <View style={{ flex: 1 }}>
                  <ThemedText size="sm" weight="semibold" numberOfLines={1}>{profile.name}</ThemedText>
                  {comparisonData && (
