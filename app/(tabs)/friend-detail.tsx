            ...
            {isListMode ? (
              <View style={styles.compactProfileHeader}>
                <View style={styles.compactAvatarContainer}>
                  {friend.imageUri ? (
                    <Image source={{ uri: friend.imageUri }} style={styles.avatar} contentFit="cover" />
                  ) : (
                    <View style={styles.avatarPlaceholder}>
                      <MaterialIcons name="people" size={18 * fontScale} color="#3b82f6" />
                    </View>
                  )}
                </View>
                <View style={{ flex: 1 }}>
                  <ThemedText size="sm" weight="semibold" numberOfLines={1}>{friend.name}</ThemedText>
                  {comparisonData && (
